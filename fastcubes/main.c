...
void processKeys(unsigned char key, int x, int y) {
	switch (key) {
		...
		case '1':
			loadModel(renderEnv->scene.root, renderEnv->scene.rootSize, 8, "../models/cac.vox",
				vectorAdd(renderEnv->scene.camera.position, vectorScale(renderEnv->scene.camera.forward, 10.0f)));
			break;
			
		case '2':
			loadModel(renderEnv->scene.root, renderEnv->scene.rootSize, 8, "../models/desklamp.vox",
				vectorAdd(renderEnv->scene.camera.position, vectorScale(renderEnv->scene.camera.forward, 10.0f)));
			break;
			
		case '3':
			loadModel(renderEnv->scene.root, renderEnv->scene.rootSize, 8, "../models/globe.vox",
				vectorAdd(renderEnv->scene.camera.position, vectorScale(renderEnv->scene.camera.forward, 10.0f)));
			break;
			
		case '4':
			loadModel(renderEnv->scene.root, renderEnv->scene.rootSize, 8, "../models/spring.vox",
				vectorAdd(renderEnv->scene.camera.position, vectorScale(renderEnv->scene.camera.forward, 10.0f)));
			break;
			
		case '5':
			loadModel(renderEnv->scene.root, renderEnv->scene.rootSize, 8, "../models/wtc.vox",
				vectorAdd(renderEnv->scene.camera.position, vectorScale(renderEnv->scene.camera.forward, 10.0f)));
			break;
	}
	...
	for (pos.z = 0; pos.z < renderEnv->scene.rootSize; pos.z++) {
		for (pos.y = 0; pos.y < renderEnv->scene.rootSize; pos.y++) {
			for (pos.x = 0; pos.x < renderEnv->scene.rootSize; pos.x++) {
				if (pos.y == 0 && pos.z == 0) {
					voxelAdd(renderEnv->scene.root, renderEnv->scene.rootSize, voxelCreate(255.0f / 255.0f, 0.0f / 255.0f, 0.0f / 255.0f), pos, 8);
				}
